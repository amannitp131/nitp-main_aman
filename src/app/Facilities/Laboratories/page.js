  ...
  const places = [
    {
      ...
      subheading: "Laboratories",
      text: `Technical knowledge is of no worth without a practical approach. Every department has laboratories that provide hands-on experience related to the subjects studied. The students are expected to take the lab courses for better learning. Various instruments, machines, and computers are provided in the labs to facilitate lab work. Lab assistants and Professors effectively guide students through their practical work. Each department has its own operational computer lab to sharpen programming skills and get hands-on experience with the necessary software. The labs are equipped with the latest technology and are periodically updated.`,
    },
    {
      image: "",
      subheading: "Workshops",
      text: `A huge workshop with all required equipment, machines, and tools is spread over a large area in the college. Work related to carpentry, blacksmithing, foundry, fitting, etc., is carried out. All activities are conducted under the guidance of experienced instructors.`,
    },
  ];

  return (
    <div className="mx-auto p-6 md:px-32 pt-12 bg-white bg-opacity-50">
      <Head>
        <title>Laboratories | NIT Patna</title>
      </Head>
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-10">
        {places.map((place, index) => (
          <div
            key={index}
            className="flex flex-col md:flex-row gap-12 items-center bg-white shadow-lg rounded-xl p-6"
          >
            {index % 2 === 0 ? (
              <>
                <div className="md:pl-4">
                  <h2 className="text-3xl font-extrabold mb-4 text-red-700">{place.subheading}</h2>
                  <p className="text-gray-700 leading-relaxed">{place.text}</p>
                </div>
                {place.image && (
                  <img
                    src={place.image}
                    alt={place.subheading}
                    className="w-3/4 md:w-2/5 rounded-xl shadow-md"
                  />
                )}
              </>
            ) : (
