      ...
      <div className="flex space-x-2 md:text-sm text-xs">
        {rightSide.map((item, index) => (
          <Link
            key={index}
            href={item.href}
            className="hover:scale-105 active:scale-95 transition-transform duration-200 ease-in-out ring-1 ring-gray-300/50 shadow-md md:shadow-lg py-2 px-1 md:px-4 md:py-2 text-xs md:text-sm rounded-xl bg-gradient-to-b from-gray-100/20 to-gray-800/30 backdrop-blur-md"
          >
            {item.text}
          </Link>
        ))}
...
export const Middle = () => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center px-6 py-1 md:py-1">
...
export const BottomNav = () => { 
  ...
  return (
    <>
    <nav className="bg-red-700 text-white backdrop-blur-xl  xl:px-6 py-3 md:flex hidden md:gap-0 xl:gap-6 items-center justify-evenly w-full shadow-lg backdrop-filter transition duration-300">
    ...
    <div className="md:hidden flex items-center justify-between bg-red-700 text-white px-4 py-3">
      ...
      >
        ...
        <div className="flex flex-col gap-2 px-6">
          {navItems.map((item, index) =>
            item.dropdown ? (
              <Dropdown key={index} title={item.name} items={item.dropdown} icon={item.icon} mobile />
            ) : (
              <Link
                key={index}
                href={item.url}
      ...
      {/* <BoldIcon
          ...
          /> */}
    {isSideMenuOpen && <MobileNav closeSideMenu={() => setSideMenuOpen(false)} navItems={navItems} />}
    </>
  ...
  return (
    <div className="mobile-nav text-black">
      <div className="mobile-nav-content text-black">
        <AiOutlineClose onClick={closeSideMenu} className="mobile-nav-close text-4xl text-black z-[1000]" />
  ...
  return (
    <div ref={animationParent} className="relative px-1 py-3 transition-all">
      ...
      <Link href={ "#"} onClick={closeSideMenu} >
        ...
        {item.dropdown && <IoIosArrowDown className={`text-xs transition-all ${isItemOpen && "rotate-180"}`} />}
      </p>
  ...
  return (
    <div className="relative px-1 py-1 transition-all ">
      ...
      {item.icon}
        ...
        {item.dropdown && <IoIosArrowDown className={`text-xs transition-all ${isSubItemOpen && "rotate-180"} `} />}
      </p>
        ...
        <div className="w-auto flex-col gap-1 bg-white py-1 transition-all text-sm">
          {item.dropdown.map((subChild, index) => (
            <p key={index} className="flex pl-4">
              {item.icon}
              <Link href={subChild.url ?? "#"} className="flex cursor-pointer items-center py-1 text-neutral-700 hover:text-black pl-1">
...
const Dropdown = ({ title, items, icon,mobile=false }) => {
  ...
  const [subDropdownPosition, setSubDropdownPosition] = useState("left-full top-0");
